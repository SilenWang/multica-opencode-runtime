...
- `opencode-go` — 使用 OpenCode Go 的 API 接口（需设置 `OPENCODE_GO_TOKEN`，可选设置 `OPENCODE_GO_BASE_URL`）。入口脚本会自动设置 `ANTHROPIC_AUTH_TOKEN=""` 并将 `ANTHROPIC_API_KEY` 设为 `OPENCODE_GO_TOKEN`。
